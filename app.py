...
class VideoCamera(object):
    def __init__(self):
        ...
        if self.send_email == 'TRUE':
            self.sender_email = 'adnaan.l@somaiya.edu'
            self.receiver_email = 'adnaan.l@somaiya.edu'
            self.password = 'yclwkfcocgqhczdk'

            self.message = MIMEMultipart("alternative")
            self.message["Subject"] = "Alert: A new person has entered the premises"
    ...
    def detect_mask(self, image):
        ...
        resized = cv2.resize(copy_img, (224, 224))
...
app.title = 'Prevention of COVID 19'
...
app.layout = html.Div(id='outer-div', children=[
    html.Div(id='banner-div', children=[
        html.Div([
            html.H2('Prevention of COVID-19 system', style={'display': 'inline-block', 'margin-top': '0px','text-decoration' : 'underline','color': 'black','letter-spacing': '2px'}),
            html.H3('DISCLAIMER : By going through the system, you give us the consent to record and store your pictures for security and quality training purposes.', style={
                    'margin-top': '0px'})
    ...
    html.Div(id='proceed-container', className='pretty_container', children=[], style={'width': '10 0%',
                                                                                       'height': '90px',
                                                                                       'text-align': 'center',
                                                                                       'padding': '0px'})
])
...
def labels_updater(n):
    ...
    if mask_detected:
        mask_label = html.H3('Mask Detected, Please gently take it down for a picture', style={'color': 'green'})
        # mask_label = html.H3('Please gently take it down for a picture', style={'color': 'black'})
...
def proceed_updater(n):
    proceed_label = html.H2('Please Stand Still!')
    if proceed:
        proceed_label = html.H2('Please Proceed', style={'color': 'green'})
    elif not_proceed:
        proceed_label = html.H2('You cannot Proceed!', style={'color': 'red'})
    elif not mask_detected:
        proceed_label = html.H2('You cannot Proceed!', style={'color': 'red'})
    elif mask_detected:
        proceed_label = html.H2('Please Align the green dot with blue dot!', style={'color': 'blue'})
